	...
	<div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
		...
		<div data-kt-swapper="true" data-kt-swapper-mode="prepend"
			...
			<h1 class="d-flex align-items-center text-dark fw-bolder fs-3 my-1">Notification
				...
				<!--end::Separator-->
				<!--begin::Description-->
				<small class="text-muted fs-7 fw-bold my-1 ms-1">Show Notification List</small>
				<!--end::Description-->
			</h1>
			<!--end::Title-->
		</div>
		...
		<div class="d-flex align-items-center py-1">
			...
			<a href="{{ route('notification-create') }}" class="btn btn-sm btn-primary" id="kt_toolbar_primary_button">Create</a>
...
<div class="container-fluid">
	...
	<div class="row">
		<div class="col-xxl-12">
			<div class="card mt-4">
				...
				<div class="d-flex justify-content-between align-items-start card-header border-0 p-1">
					<h3 class="card-title align-items-start flex-column">
						<span class="card-label fw-bolder fs-3 mb-1">Notification List</span>
						<!-- <span class="text-muted mt-1 fw-bold fs-7">Leads Form data here</span> -->
					</h3>

					<div class="d-flex flex-wrap gap-2">
						<form action="{{ route('notification-search') }}" method="POST" class="d-flex">
							@csrf
							<!--begin::Input group-->
							<div class="d-flex align-items-center position-relative">
								...
								<!--end::Svg Icon-->
								<input type="text" name="search" class="form-control form-control-sm form-control-solid w-250px ps-15" value="{{ request('search') }}" placeholder="Search Notification">
							</div>
							<!--end::Input group-->
						...
						<table class="table table-sm table-condensed table-row-gray-100 align-middle gs-0 gy-3 table-row-bordered">
							...
							<tbody>
								...
								<tr>
									...
									<td class="ps-5 text-dark fs-6">{{($notifications->currentPage() - 1) * $notifications->perPage() + $loop->iteration}}</td>
									<td class="ps-5 text-dark fs-6 w-300px">
										@if($notification->lead_first_name && $notification->lead_last_name)
										{{ $notification->lead_first_name . ' ' . $notification->lead_last_name . ' <' . $notification->lead_email . '>' }}
										@else
										
										@endif
									</td>

									<td class="text-dark fs-6">{{ \Carbon\Carbon::parse($notification->notify_datetime)->format('Y-m-d h:i A') }}</td>
									<td class="text-dark fs-6 w-300px">
										@if($notification->user_first_name && $notification->user_last_name)
			...
			@else
				<li class="page-item previous"><a href="{{ $notifications->previousPageUrl() }}" class="page-link"><i class="previous"></i></a></li>
			@endif
			...
			@for ($page = 1; $page <= $notifications->lastPage(); $page++)
				...
				@else
				<li class="page-item"><a href="{{ $notifications->url($page) }}" class="page-link">{{ $page }}</a></li>
				@endif
				@endfor

				<!-- Next Page Link -->
				@if ($notifications->hasMorePages())
				<li class="page-item next"><a href="{{ $notifications->nextPageUrl() }}" class="page-link"><i class="next"></i></a></li>
				@else
